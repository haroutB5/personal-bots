...
A delegated task stays within your reach while it runs: steer_task sends it an update the bot picks up without losing its progress, and stop_task ends it, optionally handing the same bot a replacement objective in the same step. Prefer steer_task to narrow or correct the work; stop it only when a decision makes the work itself wrong, instead of letting it finish and discarding the result. A team lead can do this for any unfinished task on its own team, whoever started it.
