...
describe("personalBotSystemInstructions", () => {
  it("routes memory, secrets and browsing to the app's tools", () => {
    const text = personalBotSystemInstructions("You are Coach.");

    assert.isTrue(text.startsWith("You are Coach.\n\n<app_rules>"));
  ...
  it("still gives a bot with blank instructions the app rules", () => {
    const text = personalBotSystemInstructions("   ");

    assert.isTrue(text.startsWith("<app_rules>"));
  ...
  it("lands inside the bot instructions block every adapter sends", () => {
    const block = withBotInstructions(
      ...
      personalBotSystemInstructions("Be brief."),
    );
