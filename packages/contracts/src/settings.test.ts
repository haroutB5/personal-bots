...
describe("ClientSettings rich text composer", () => {
  // Decision rich-text-composer-default: rich text stays opt-in here, so this
  // fails if upstream's on-by-default lands again.
  it("leaves rich text off for new and existing settings without a saved preference", () => {
    expect(decodeClientSettings({}).composerRichTextEnabled).toBe(false);
    expect(decodeClientSettings({ sendShortcut: "mod-enter" }).composerRichTextEnabled).toBe(false);
  });

  it("preserves an explicit preference through patches and persistence", () => {
    for (const enabled of [true, false]) {
      const preference = { composerRichTextEnabled: enabled };
